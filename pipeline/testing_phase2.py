...
def loading_checkpoints_and_testing(args):
    ...
    model_ckpt = "/home/shiquan/Projects/deBiasing-Dialogue/Dialogue_Annotator/pipeline/entity-prediction-phase2-models/entity-prediction_ckpt_epoch_1.ckpt"
    ...
    print(f"Testing {hparams['name']} models")
    ...
    OUTPUT_DIR = f"{DIALOGUE_EXPERIMENTS_DIR}/{hparams['treatment']}/COMPARE"
    trainer = Trainer(gpus=1 if DEVICE.type == "cuda" else 0,
                      default_save_path=OUTPUT_DIR,
                      show_progress_bar=True)
    hparams['output_path'] = trainer.logger.experiment.log_dir.rstrip('tf')
    logger = init_logger("testing", hparams['output_path'])
    hparams["bert_params"]["batch_size"] = hparams["batch_size"]
    # Load model from saved checkpoints
    # checkpoint = torch.load(model_ckpt, map_location=lambda storage, loc: storage)
    logger.info(f"Loading model for {hparams['bert_params']['name']} from: {model_ckpt}")
    model = DebiasedBertPretrainedClassifier.load_from_checkpoint(model_ckpt)
    # Overide model hyperparameters
    # model.bert_classifier.name = hparams['bert_params']['name']
    model.hparams.max_seq_len = hparams["max_seq_len"]
    model.hparams.data_path = hparams["data_path"]
...
def main():
    ...
    loading_checkpoints_and_testing(args)
