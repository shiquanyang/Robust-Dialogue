...
from Dialogue_Annotator.lm_finetune.bert_dst_finetune_models_w_annotator_id import BertForDSTPretraining, BertForDSTwControlPreTraining
...
DSTInputFeatures = namedtuple("InputFeatures", "input_ids input_mask lm_label_ids user_intent_labels annotator_id_labels dst_labels")
...
class PregeneratedDSTDataset(Dataset):
    def __init__(self, training_path, epoch, tokenizer, lang, num_data_epochs, reduce_memory=False):
        ...
        if reduce_memory:
            ...
            annotator_id_labels = np.zeros(shape=(num_samples, 1), dtype=np.int32)
        ...
        with data_file.open() as f:
            for i, line in enumerate(tqdm(f, total=num_samples, desc="Training examples")):
                ...
                annotator_id_labels[i] = features.annotator_id_labels
        ...
        self.annotator_id_labels = annotator_id_labels
    ...
    def __getitem__(self, item):
        return (torch.tensor(self.input_ids[item].astype(np.int64)),
                ...
                torch.tensor(self.annotator_id_labels[item].astype(np.int64)),
    ...
    def convert_example_to_features(example, tokenizer, max_seq_length):
        ...
        annotator_id_labels = example["annotator_id_labels"]
...
def dst_pretrain(args, lang):
    ...
    if args.control_task:
        config = BertConfig.from_pretrained(args.bert_model, num_labels=lang.n_annotators)
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=len(train_dataloader), desc=f"Epoch {epoch}") as pbar:
            for step, batch in enumerate(train_dataloader):
                ...
                input_ids, input_mask, lm_label_ids, user_intent_labels, annotator_id_labels, bus_leaveat_labels, \
                ...
                loss = state_tracking_loss
                if args.control_task:
                    # loss_f_control = BCELoss()
                    loss_f_control = CrossEntropyLoss()
                    annotator_id_loss = loss_f_control(outputs[36].view(-1, lang.n_annotators), annotator_id_labels.view(-1))
                    loss += annotator_id_loss

                mlm_loss = masked_lm_loss
                adversarial_loss = state_tracking_loss
                if args.control_task:
                    control_loss = annotator_id_loss
                if n_gpu > 1:
                ...
                if args.control_task:
                    pbar.set_postfix_str(f"Loss: {mean_loss:.5f}, MLM_Loss: {mlm_loss:.5f}, Adversarial_Loss: {adversarial_loss:.5f}, AnnotatorID_Loss: {control_loss:.5f}")
                else:
...
def main():
    ...
    # lang_path = '/Users/shiquan/PycharmProjects/deBiasing-Dialogue/Dialogue_Annotator/datasets/MultiWOZ_2.2/MultiWOZ_Lang_w_Annotator.pkl'
    # lang_path = '/home/shiquan/Projects/deBiasing-Dialogue/Dialogue_Annotator/datasets/MultiWOZ_2.2/MultiWOZ_Lang.pkl'
    # lang_path = '/home/shiquan/Projects/deBiasing-Dialogue/Dialogue_Annotator/datasets/MultiWOZ_2.2/MultiWOZ_Lang_w_Annotator_p=0_5.pkl'
    lang_path = '/home/yimeng/shiquan/deBiasing-Dialogue/Dialogue_Annotator/datasets/MultiWOZ_2.2/MultiWOZ_Lang_w_Annotator_p=0_5.pkl'
